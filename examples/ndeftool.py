...
def make_smartposter_parser(parser):
    ...
    parser.add_argument(
            "-t", metavar="TITLE", dest="titles", action="append",
            default=list(),
            help="smartposter title as '[language:]titlestring'")
    parser.add_argument(
            "-i", metavar="ICON", dest="icons", action="append",
            default=list(),
...
def make_smartposter(args):
    ...
    for icon in args.icons:
        ...
        if args.action not in ('default', 'exec', 'save', 'edit'):
...
def make_wifipassword(args):
    if args.password is None:
        printable = ''.join(string.printable.split())
        args.password = ''.join([random.choice(printable) for _ in range(32)])
...
def make_bluetoothcfg_parser(parser):
    ...
    parser.add_argument(
            "-s", dest="service", metavar="STRING", action="append",
            default=[],
...
def make_bluetoothcfg(args):
    ...
    for index, service in enumerate(args.service):
        for key, value in nfc.ndef.bt_record.service_class_uuid_map.items():
            ...
            try:
                record.service_class_uuid_list = [service, ]
            except ValueError:
...
def add_cat_parser(parser):
    ...
    parser.add_argument(
            "records", metavar="record", type=argparse.FileType('r'),
            nargs="+",
            help="record file")
...
if __name__ == '__main__':
    ...
    add_print_parser(s.add_parser(
            'print', help='parse and print messages'))
    add_make_parser(s.add_parser(
            'make', help='create ndef messages'))
    add_pack_parser(s.add_parser(
            'pack', help='pack data into an ndef record'))
    add_split_parser(s.add_parser(
            'split', help='split messages into records'))
    add_cat_parser(s.add_parser(
            'cat', help='concatenate records to message'))

    a = p.parse_args()

    verbosity = logging.INFO if a.verbose else logging.ERROR
    verbosity = logging.DEBUG if a.debug else verbosity
