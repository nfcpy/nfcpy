...
def raw_access_point_wait_recv(socket, timeout=3.0):
    ...
    if not socket.poll("recv", timeout):
        raise TestFail(
            "expected data but got none within %d seconds" % timeout)
    pdu = socket.recv()
...
def raw_access_point_wait_no_recv(socket, timeout=3.0):
    ...
    if socket.poll("recv", timeout):
        raise TestFail(
            "expected none but got data within %d seconds" % timeout)
...
class TestProgram(CommandLineInterface):
    ...
    def on_llcp_startup(self, llc):
        ...
        try:
            on_startup = eval("self." + func_name)
    ...
    def on_llcp_connect(self, llc):
        ...
        try:
            on_connect = eval("self." + func_name)
        except AttributeError:
            pass
        else:
            llc = on_connect(llc)
    ...
    def exec_TC_CTO_TAR_BI_02_x(self, llc, x):
        ...
        if x != 4:
            assert pdu.reason == 2, "DM reason code not 02h"
        else:
            assert pdu.reason in (3, 16), "DM reason code not 03h or 10h"
        socket.close()
    ...
    def exec_TC_CTO_TAR_BI_03_x(self, llc, x):
        ...
        payload = struct.pack('!BBB25s', 8, 26, 1,
                              b'urn:nfc:sn:dta-co-echo-in')
    ...
    def test_TC_CTO_INI_BV_02(self, llc):
        ...
        recv_socket.poll("recv", 5)
        assert recv_socket._tco.state.SHUTDOWN, \
            "outbound connection not closed"
