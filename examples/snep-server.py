...
class SonyStcSnepServer(nfc.snep.SnepServer):
    def __init__(self):
        super(SonyStcSnepServer, self).__init__('urn:nfc:xsn:sony.de:snep')
...
def main():
    ...
    try:
        default_snep_server = DefaultSnepServer()
        sonystc_snep_server = SonyStcSnepServer()
        default_snep_server.start()
        sonystc_snep_server.start()
        while True:
            time.sleep(1)
