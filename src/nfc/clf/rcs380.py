...
class Device(device.Device):
    ...
    def sense_tta(self, target):
        ...
        try:
            ...
            if target.sel_req:
                ...
                for i, sel_cmd in zip(range(0, len(uid), 4),
                                      bytearray(b"\x93\x95\x97")):
                    sel_req = bytearray(
                        struct.pack("B", sel_cmd) + b"\x70" + uid[i:i+4])
        ...
        while recv_timeout > 0:
            ...
            if len(data) == 13 and data[7] == 6 and data[8] == 0:
                ...
                if (((sensf_req[1] == 255 or sensf_req[1] == sensf_res[17]) and
                    ...
                    if sensf_req[3] == 2:
                        transmit_data += b"\x00"
                        transmit_data += struct.pack(
                            "B", 1 << (target.brty == "424F"))
                    transmit_data = struct.pack(
                        "B", len(transmit_data)+1) + transmit_data
    ...
    def listen_dep(self, target, timeout):
        ...
        def send_res_recv_req(brty, data, timeout):
            if data:
                data = (b"", b"\xF0")[brty == b"106A"] \
                    + struct.pack("B", len(data)+1) + data
