...
class Topaz(tt1.Type1Tag):
    ...
    def _format(self, version, wipe):
        ...
        if wipe is not None:
            tag_memory[14:104] = 90 * pack("B", wipe & 0xFF)
...
class Topaz512(tt1.Type1Tag):
    ...
    def _format(self, version, wipe):
        tag_memory = tt1.Type1TagMemoryReader(self)
        tag_memory[8:16] = unhexlify(b"E1103F000103F230")
        tag_memory[16:24] = unhexlify(b"330203F002030300")
        ...
        if wipe is not None:
            tag_memory[24:104] = 80 * pack("B", wipe & 0xFF)
            tag_memory[128:512] = 384 * pack("B", wipe & 0xFF)
