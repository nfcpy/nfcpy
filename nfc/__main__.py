...
def usb_device_access_denied(bus, dev, vid, pid, path):
    ...
    if platform.system().lower() == "linux":
        ...
        if not os.access(devnode, os.R_OK | os.W_OK):
            ...
            try: plugdev = grp.getgrnam("plugdev")
            except KeyError: plugdev = None
    ...
    if platform.system().lower() == "linux":
        ...
        if os.access(sysfs_config_entry + "nfc", os.F_OK):
            ...
            try:
                ...
                lsof = "lsof -t " + devnode
                try: pid = subprocess.check_output(lsof.split()).strip()
                except subprocess.CalledProcessError: pid = None
                if pid is not None:
